      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-16">
          {problems.map((problem, index) => (
            <div
            key={index}
            className="border border-transparent bg-[#fffaf4] text-black px-3 py-2 rounded-xl shadow-sm hover:shadow-lg hover:scale-[1.02] transition-all duration-200 ease-in-out [background:linear-gradient(#fffaf4,rgba(255,250,244,0.9))_padding-box,linear-gradient(135deg,rgba(255,165,0,0.6),rgba(255,200,100,0.4))_border-box]"
            >
            <div className="flex items-center gap-3">
              {/* Icon background */}
              <div
                className={`w-10 h-10 flex-shrink-0 rounded-full flex items-center justify-center shadow-md bg-gradient-to-tr from-${problem.color}-500 to-${problem.color}-400`}
              >
                <problem.icon className={`w-5 h-5 text-${problem.color}`} />
              </div>
          
              {/* Text */}
              <p className="text-gray-800 font-medium text-[10px] leading-snug">
                {problem.text}
              </p>
            </div>
          </div>
        ...
        <div className="bg-[#fffaf4] text-black p-8 sm:p-12 rounded-2xl shadow-sm mb-12">
          ...
          <div className="text-center">
            ...
            <button
              ...
              className="bg-gray-800 text-white font-bold py-4 px-8 rounded-full hover:bg-orange-700 transition-all duration-300 transform hover:scale-105 shadow-lg"
            >
              Claim free audit
            </button>
          </div>
